...
const Page = () => {
  ...
  const updateDelegate = (index, value) => {
  ...
  const validateForm = () => {
    if (!delegationName.trim()) {
      toast.error("Delegation Name is required");
      return false;
    }
    ...
    if (!headChaperone.trim()) {
      toast.error("Head Chaperone name is required");
      return false;
    }

    const validDelegates = delegates.filter((d) => d.trim() !== "");
    if (validDelegates.length < MIN_DELEGATES) {
      toast.error(`Please enter names for at least ${MIN_DELEGATES} delegates`);
      return false;
    }

    if (additionalEmail && !/\S+@\S+\.\S+/.test(additionalEmail)) {
      toast.error("Additional email is invalid");
      return false;
    }
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          ...
          {!submitted ? (
            ...
            >
              ...
              <Grid>
                <Field
                  label="Head Chaperone Name"
                  required
                  value={headChaperone}
                  onChange={(e) => setHeadChaperone(e.target.value)}
                />
                <Field
                  label="Chaperone 2 (optional)"
                  value={chaperone2}
                  onChange={(e) => setChaperone2(e.target.value)}
                />
                <Field
                  label="Chaperone 3 (optional)"
                  value={chaperone3}
                  onChange={(e) => setChaperone3(e.target.value)}
                />
              </Grid>

              <div>
                <h2 className="font-semibold mb-3 text-lg">
                  Delegates (Minimum {MIN_DELEGATES} required)
                </h2>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-red-600 to-indigo-700 cursor-pointer text-white py-3 rounded-lg"
              >
            ...
            <div className="bg-white rounded-2xl p-10 text-center">
              ...
              <p>Total Delegates:{" "}
                <b>{delegates.filter((d) => d.trim()).length}</b>
              </p>
