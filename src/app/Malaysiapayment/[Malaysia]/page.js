...
export default function Home() {
  ...
  /* ================= DESTINATION ================= */
  useEffect(() => {
    setCheck('Kuala Lumpur, Malaysia');
  }, [setCheck]);
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `https://world-diplomats-backend.onrender.com/api/fournames?filters[id][$eq]=${id}`
        );
  ...
  const handleCreateInvoice = async (amount) => {
    ...
    let disnew = 'Basic';
    if (amount === 678) disnew = 'Shepandum Experience';
    if (amount === 809) disnew = 'Zagatiya Experience';
        ...
        <section className="relative z-10 py-28">
          <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-center text-4xl font-bold text-white mb-4">
              Pricing for Kuala Lumpur, Malaysia
            </h2>
            ...
            >
              <PlanCard
                ...
                onPay={() => seo(501)}
                onInvoice={() => handleCreateInvoice(501)}
              ...
              <PlanCard
                ...
                setActiveCard={setActiveCard}
                onPay={() => seo(678)}
                onInvoice={() => handleCreateInvoice(678)}
                loader={invoiceLoader}
              />
              ...
              <PlanCard
                ...
                setActiveCard={setActiveCard}
                onPay={() => seo(809)}
                onInvoice={() => handleCreateInvoice(809)}
                loader={invoiceLoader}
              />
            </div>
          </div>
...
function PlanCard({
  ...
  return (
    <div className={`bg-white rounded-3xl p-8 flex flex-col shadow-lg ${badge ? 'border-2 border-red-600 relative' : ''}`}>
      ...
      <p className="text-center text-4xl font-extrabold text-blue-800 mt-4">
        ${price}
      </p>
