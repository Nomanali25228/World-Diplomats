...
const RegisterNowClient = () => {
  ...
  useEffect(() => {
    if (delegateDetails.length !== delegates.length) {
      setTimeout(() => {
  ...
  }, [delegates.length, delegateDetails.length]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const customerResponse = await fetch("/api/get-or-create-customer", {
        method: "POST",
      ...
      const customerId = customerJson.customerId || customerJson.customer?.id;

      if (!customerId) {
        toast.error("Customer ID not returned from server.");
        setSubmitting(false);
        return;
      }
      ...
      const Id = resultText?.data?.id;

      try {
        const emailEndpoint = DEST_EMAIL_MAP[form.destination] || "/api/register";
      ...
      await cronjob({
        id: Id,
        name: form.name || "Group Registration",
...
export default RegisterNowClient
