...
export async function POST(request) {
    try {
        ...
        // Inline destination-specific values (no external emailTemplates.js)
        const dest = (destination || 'Dubai, UAE').trim();
        let desname = dest;
        let country = 'UAE';
        let CityTour = '';
        if (dest.includes('Dubai')) { desname = 'Dubai, UAE'; country = 'Dubai, UAE'; CityTour = 'Desert Safari'; }
        ...
        const buttonLink = isDelegation ? 'https://wa.me/+447490344639' : `https://www.worlddiplomats.org/Dubaipayment/1?userid=${id}`;
