...
export default function Home() {
  ...
  const { amounts } = useDestination();
  const router = useRouter();

  // Redirect if invalid amount
  useEffect(() => {
    // Allow navigation when either the context `amounts` or localStorage `amounts` is valid
    const stored = localStorage.getItem("amounts");
    const storedAmount = stored ? parseFloat(stored) : null;
    const isStoredValid = storedAmount !== null && !Number.isNaN(storedAmount) && storedAmount > 0;
    const isContextValid = amounts && amounts > 0;

    if (!isStoredValid && !isContextValid) {
      router.push("/");
    }
  }, [amounts, router]);
